...
const expectedPublicPackageNames = new Set([
...
const actualPublicPackageNames = new Set(publicPackages.map((packageJson) => packageJson.name));
const missingPackages = [...expectedPublicPackageNames].filter((name) => !actualPublicPackageNames.has(name));
const unexpectedPackages = publicPackages.filter(
  (packageJson) => !packageJson.name || !expectedPublicPackageNames.has(packageJson.name)
);

if (missingPackages.length > 0 || unexpectedPackages.length > 0) {
  const unexpectedNames = unexpectedPackages.map((packageJson) => packageJson.name ?? '<unnamed>');

  throw new Error(
    `Public package set does not match the coordinated release group. Missing: ${missingPackages.join(', ') || 'none'}. Unexpected: ${unexpectedNames.join(', ') || 'none'}`
  );
}
