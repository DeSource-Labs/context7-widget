<template>
  <div ref="containerRef" class="grid-scan" />
</template>
...
const srgbColor = (hex: string): THREE.Color => new THREE.Color(hex).convertSRGBToLinear();
...
const smoothDampVec2 = (
  ...
  const maxChange = maxSpeedVal * smoothTime;
...
const setupAnimation = () => {
  ...
  const pushScan = (time: number): void => {
    if (scanStarts.length >= maxScans) {
      scanStarts.shift();
    }
    ...
    for (let i = 0; i < scanStarts.length && i < maxScans; i += 1) {
      buffer[i] = scanStarts[i];
    }
  ...
  const tick = (): void => {
    ...
    const tiltSmoothed = smoothDampFloat(tiltCurrent, tiltTarget, { v: tiltVel }, smoothTime, Infinity, dt);
    tiltCurrent = tiltSmoothed.value;
    tiltVel = tiltSmoothed.v;

    const yawSmoothed = smoothDampFloat(yawCurrent, yawTarget, { v: yawVel }, smoothTime, Infinity, dt);
    yawCurrent = yawSmoothed.value;
    yawVel = yawSmoothed.v;
